...
db.get("/getUserEntries", async (req, res) => {
  if (collection === null) {
    ...
    let users = await collection
      .find({ uniqueId: req.query.uniqueId })
      .toArray();
    if (users.length > 0) {
      console.log("============== User Entries ==============");
      console.log(users[0].entries);
      console.log("============ End User Entries ============");
      console.log("============ User Categories =============");
      console.log(users[0].categories);
      console.log("=========== End User Categories ==========");
      ...
      try {
        ...
        console.log("Failed to retrieve user's entries: " + e.message);
...
db.post("/addEntry", async (req, res) => {
  ...
  let subcategory = req.body.isAspiration
    ? "Aspiration: " + req.body.subcategory
...
db.post("/addCategory", async (req, res) => {
  ...
  userCategories.push(req.body.newCategory);
  ...
  try {
    ...
    console.log("Failed to add an entry!" + e.message);
    return res.redirect("/dashboard");
  }
});
